...
bool TicTacToeGame::checkForVictory(char currentPlayer){
	...
	if ((board[0][0] == currentPlayer) && (board[0][0] == board[1][1]) && (board[1][1] == board[2][2])) {	//Top left diagonal
		return true;
	}

	if ((board[2][0] == currentPlayer) && (board[2][0] == board[1][1]) && (board[1][1] == board[0][2])) {	//Top right diagonal
		return true;
	}
